package edu.bu.pas.pokemon.generators;
...
/**
 * Custom BattleCreator that excludes Metronome move to prevent crashes with DRAC agent.
 * This class overrides the default BattleCreator from the pokePA jar.
 */
public class BattleCreator extends Object {
    ...
    public static Move[] chooseRandomUniqueMoves(int dexIdx, int size, Random rng, CoreRegistry cr) {
        ...
        Set<String> moveSet = cr.getMoveSet(dexIdx);
        Set<String> allMoveNames = cr.getAllMoveNames();

        // Filter out metronome to prevent DRAC agent crashes
        Set<String> availableMoves = new HashSet<String>();
        for (String moveName : moveSet) {
            if (moveName.equalsIgnoreCase("metronome")) {
                continue;
            }
            if (allMoveNames.contains(moveName)) {
                availableMoves.add(moveName);
            }
        }

        List<String> moveSetList = new ArrayList<>(availableMoves);
        for (int idx = 0; idx < size && moveSetList.size() > 0; ++idx) {
    ...
    public static Battle makeRandomTeams(int team1Size, int team2Size, int numMovesPerPokemon, Random rng,
                                         Agent team1Agent, Agent team2Agent) throws IOException {
        ...
        for (int teamIdx = 0; teamIdx < teams.length; ++teamIdx) {
            ...
            for (int pokemonIdx = 0; pokemonIdx < pokemon.length; ++pokemonIdx) {
                ...
                Pokemon p = cr.spawnPokemon(
                        dexIdx,
                        100,
                        new int[]{15, 15, 15, 15, 15, 15, 1, 1},
                        new int[]{65535, 65535, 65535, 65535, 65535, 65535, 1, 1},
                        moves
                );
